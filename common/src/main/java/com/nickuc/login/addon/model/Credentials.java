...
public class Credentials {
    ...
    public synchronized User getUser(Platform platform) {
        return getUser(platform.getPlayerName());
    }
    ...
    public synchronized User getUser(String username) {
        ...
        User user = new User(username, new ArrayList<>(), new HashMap<>());
        users.add(user);
        return user;
    }
    ...
    public static class Server {
        ...
        public static Server fromJson(JsonObject json) {
            if (json.has("uuid") && json.has("password")) {
                ...
                PublicKey publicKey = json.has("publicKey") ? RSA.getPublicKeyFromBytes(Base64.getDecoder().decode(json.get("publicKey").getAsString())) : null;
